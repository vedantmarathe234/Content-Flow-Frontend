...
const AdminDashboard = () => {
  ...
  const total =
    stats.approved + stats.rejected + stats.pendingLeader + stats.pendingAdmin;
    ...
    <div className="space-y-6 p-2 bg-slate-50/50 min-h-screen font-sans">
        ...
        {[
          {
            ...
            borderColor: "border-l-[#063A3A]",
            iconColor: "text-[#063A3A]",
            iconBg: "bg-[#063A3A]/5",
          },
          {
            title: "Total Teams",
            val: stats.totalTeams,
            icon: <Users size={20} />,
            borderColor: "border-l-[#0D7A80]",
            iconColor: "text-[#0D7A80]",
            iconBg: "bg-[#0D7A80]/5",
          },
          {
            title: "Pending Content",
            val: stats.pendingLeader + stats.pendingAdmin,
            icon: <Clock size={20} />,
          ...
          <div
            key={i}
            className={`bg-white p-5 rounded-xl border border-slate-200/80 border-l-4 ${card.borderColor} shadow-sm flex items-center justify-between transition-all hover:shadow-md`}
          >
            <div>
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                {card.title}
              </p>
              <h3 className="text-2xl font-extrabold text-[#063A3A] mt-1">
                {card.val}
              </h3>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-l-4 border-l-[#0D7A80] border-slate-200/80 shadow-sm flex flex-col">
          <h3 className="text-sm font-bold text-[#063A3A] mb-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-2xl border border-l-4 border-l-[#0D7A80] border-slate-200/80 shadow-sm bg-[#0D7A80]/[0.01]">
          <h3 className="text-sm font-bold text-[#063A3A] mb-5">
            Top Departments
          </h3>
          <div className="space-y-3">
            {!stats.topDepartments || stats.topDepartments.length === 0 ? (
              ...
              stats.topDepartments.map((dept, i) => (
                <div
                  key={i}
                  className="flex justify-between items-center text-sm p-2.5 rounded-xl bg-white border border-slate-100 shadow-2xs"
                >
          ...
          <div className="space-y-3  max-h-[380px] overflow-y-auto scrollbar scrollbar-thumb-[#0D7A80]/40 custom-scrollbar">
            {recentActivities.length === 0 ? (
              ...
              recentActivities.map((act, i) => (
                <div
                  key={i}
                  className="flex items-start gap-3 p-2.5 rounded-xl bg-white border border-slate-100 shadow-2xs"
                >
